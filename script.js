...
window.onload=function(){
  ...
  if(DB.settings&&DB.settings.payNums) PAYMENT_NUMBERS={...PAYMENT_NUMBERS,...DB.settings.payNums};
  const saved=localStorage.getItem('eagle_user');
  if(saved){
    state.user=JSON.parse(saved);
    // Reload wallet AND name from DB (in case admin changed it)
    const u=DB.users.find(u=>u.email===state.user.email);
    if(u){
      state.user.wallet=u.wallet;
      state.user.name=u.name; // sync name from DB
    }
...
function doLogin(){
  ...
  // Use existing name from DB, fallback to email-based name
  const name=existing?existing.name:e.split('@')[0].replace(/\./g,' ').replace(/\b\w/g,c=>c.toUpperCase());
...
function googleLogin(){
  const inputName=prompt('আপনার নাম লিখুন (Enter your name):','');
  const name=(inputName&&inputName.trim())?inputName.trim():'Player_'+Math.floor(Math.random()*9000+1000);
...
function savePaymentNumbers(){
  PAYMENT_NUMBERS.bkash=document.getElementById('numBkash').value.trim()||PAYMENT_NUMBERS.bkash;
  PAYMENT_NUMBERS.nagad=document.getElementById('numNagad').value.trim()||PAYMENT_NUMBERS.nagad;
  PAYMENT_NUMBERS.rocket=document.getElementById('numRocket').value.trim()||PAYMENT_NUMBERS.rocket;
  PAYMENT_NUMBERS.upay=document.getElementById('numUpay').value.trim()||PAYMENT_NUMBERS.upay;
  DB.settings.payNums={...PAYMENT_NUMBERS};
  saveDB();
